...
export interface BonusPool {
  ...
  status: 'draft' | 'calculated' | 'allocated' | 'paid'
  createdAt: string
  updatedAt: string
}
...
// 验证绩效数据
export function validatePerformanceData(period: string) {
  return request({
    url: '/calculations/bonus-calculations/validate-performance',
    method: 'get',
    params: { period }
  })
}
...
// 发放奖金
export function payBonusPool(poolId: number) {
  return request({
    url: `/calculations/bonus-pools/${poolId}/payment`,
    method: 'post'
  })
}