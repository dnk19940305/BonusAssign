...
export function updateSystemConfig(config: Partial<SystemConfig>) {
  return request({
    ...
    method: 'put',
    data: { configs: config }  // 包装在configs属性中
  })
}
...
export function exportSystemConfig() {
  return request({
    url: '/system/configs/export',
    method: 'get'
  })
}

// 导入系统配置
export function importSystemConfig(data: any) {
  return request({
    url: '/system/configs/import',
    method: 'post',
    data
  })
}
