  ...
  <div class="system-config">
    <div class="page-header">
      ...
      <div class="header-actions">
        <PermissionButton :permissions="['admin','*']" type="primary" @click="saveAllConfigs" :loading="saving">
          <el-icon><Check /></el-icon>
          保存所有配置
        </PermissionButton>
        <PermissionButton :permissions="['admin','*']" @click="resetConfigs">
            ...
            <el-form :model="calculationConfig" label-width="140px">
              <el-form-item label="默认算法">
                <el-select v-model="calculationConfig.defaultAlgorithm" placeholder="请选择算法">
                  <el-option label="三维奖金计算" value="three_dimensional" />
              ...
              <el-form-item label="舍入规则">
                <el-select v-model="calculationConfig.roundingRule" placeholder="请选择舍入规则">
                  <el-option label="四舍五入（向上）" value="round_half_up" />
...
const loadConfigs = async () => {
  ...
  try {
    ...
    if (configs.calculation) {
      // 转换timeout从秒到毫秒显示
      Object.assign(calculationConfig, {
        ...configs.calculation,
        timeout: Math.floor(configs.calculation.timeout / 1000)
      })
    }
    if (configs.notification) {
      ...
      Object.assign(notificationConfig, {
        email: {
          enabled: configs.notification.emailEnabled || false,
          smtpHost: configs.notification.smtpHost || '',
          smtpPort: configs.notification.smtpPort || 587,
          senderEmail: configs.notification.senderEmail || '',
          events: configs.notification.emailEvents || []
        },
        system: {
          enabled: configs.notification.systemNotificationEnabled || false,
          retentionDays: configs.notification.notificationRetentionDays || 30,
          maxCount: configs.notification.maxNotificationCount || 1000
        }
      })
    }
    if (configs.security) {
      Object.assign(securityConfig, {
        password: {
          minLength: configs.security.passwordMinLength || 8,
          requirements: configs.security.passwordRequirements || ['uppercase', 'lowercase', 'number'],
          expireDays: configs.security.passwordExpireDays || 90
        },
        login: {
          maxFailures: configs.security.maxLoginFailures || 5,
          lockoutDuration: configs.security.lockoutDuration || 30
        },
        session: {
          timeout: Math.floor((configs.security.sessionTimeout || 7200) / 60),
          rememberMe: configs.security.enableRememberMe !== false,
          singleSignOn: configs.security.enableSingleSignOn || false
        },
        access: {
          enableWhitelist: configs.security.enableIpWhitelist || false,
          whitelist: configs.security.ipWhitelist || ''
        }
      })
    }
...
const saveAllConfigs = async () => {
  ...
  try {
    ...
    const configs: any = {
      ...
      calculation: {
        ...calculationConfig,
        timeout: calculationConfig.timeout * 1000 // 转换为毫秒
      },
      notification: {
        ...
        senderEmail: notificationConfig.email.senderEmail,
        emailEvents: notificationConfig.email.events,
        systemNotificationEnabled: notificationConfig.system.enabled,
        notificationRetentionDays: notificationConfig.system.retentionDays,
        maxNotificationCount: notificationConfig.system.maxCount
      },
      security: {
        passwordMinLength: securityConfig.password.minLength,
        passwordRequirements: securityConfig.password.requirements,
        passwordExpireDays: securityConfig.password.expireDays,
        maxLoginFailures: securityConfig.login.maxFailures,
        lockoutDuration: securityConfig.login.lockoutDuration,
        sessionTimeout: securityConfig.session.timeout * 60, // 转换为秒
        enableRememberMe: securityConfig.session.rememberMe,
        enableSingleSignOn: securityConfig.session.singleSignOn,
        enableIpWhitelist: securityConfig.access.enableWhitelist,
        ipWhitelist: securityConfig.access.whitelist
      }
    }
...
const exportConfigs = async () => {
  ...
  try {
    ...
    ElMessage.error('配置导出失败: ' + (error.message || '未知错误'))
  } finally {
...
const importConfigs = async () => {
  ...
  try {
    ...
    // 调用导入API
    await importSystemConfig(importData)
    ...
    ElMessage.error('配置导入失败：' + (error.message || '未知错误'))
  } finally {
...
const validateConfigs = async () => {
  try {
    ...
    const response = await validateSystemConfig(configs)
    
    if (response.data.valid) {
      ElMessage.success('配置验证通过')
    } else {
      const errors = response.data.errors.map((e: any) => e.message).join('\n')
      ElMessage.error('配置验证失败：\n' + errors)
    }
  } catch (error: any) {
    console.error('验证配置失败:', error)
    ElMessage.error('配置验证失败: ' + (error.message || '未知错误'))
  }
}
