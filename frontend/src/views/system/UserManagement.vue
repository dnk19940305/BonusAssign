    ...
    <el-card>
      <template #header>
        <div class="card-header">
          <span>用户管理</span>
          <el-button 
            v-if="userStore.hasAnyPermission(['user:create', 'admin', '*'])"
            type="primary" 
      ...
      <div 
        class="batch-actions" 
        v-show="selectedUsers.length > 0 && userStore.hasAnyPermission(['user:update', 'admin', '*'])"
      >
        <el-button 
          v-if="userStore.hasAnyPermission(['user:update', 'admin', '*'])"
          type="success" 
          @click="batchEnable"
        >
          批量启用
      ...
      >
        ...
        <el-table-column label="操作" width="200" fixed="right">
          <template #default="{ row }">
            ...
            <el-button 
              v-if="userStore.hasAnyPermission(['user:reset_password', 'admin', '*'])"
              type="warning" 
              size="small" 
            ...
            <el-button 
              v-if="userStore.hasAnyPermission(['user:delete', 'admin', '*']) && row.id !== userStore.user?.id"
              type="danger" 
              size="small" 
...
interface User {
  ...
  email?: string
  phone?: string
  role_id: string // 改为字符串类型，与NeDB的_id字段一致
...
const handleDelete = async (row: User) => {
  try {
    await ElMessageBox.confirm(`确定要删除用户"${row.real_name}"吗？`, '确认删除', {
