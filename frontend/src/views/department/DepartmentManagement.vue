  ...
  <div class="page-container">
    <div class="page-header  flex-between">
      ...
      <div class="header-actions">
        <el-button 
          v-if="userStore.hasAnyPermission(['department:create', 'admin', 'hr', '*'])"
          type="primary" 
        ...
        <el-button 
          v-if="userStore.hasAnyPermission(['department:export', 'admin', 'hr', '*'])"
          @click="handleExport"
        >
    ...
    <el-card class="search-card" v-if="viewMode === 'list'">
      <el-form :model="queryForm" inline>
        ...
        <el-form-item label="业务线">
          <el-select v-model="queryForm.businessLineId" placeholder="请选择业务线"  style="width: 150px;">
            <el-option
        ...
        <el-form-item label="状态">
          <el-select v-model="queryForm.status" placeholder="请选择状态"  style="width: 100px;">
    ...
    <el-card 
      class="batch-actions" 
      v-if="selectedDepartments.length > 0 && viewMode === 'list' && userStore.hasAnyPermission(['department:update', 'admin', 'hr', '*'])"
    >
      <el-alert
      ...
      <div class="batch-buttons">
        <el-button 
          v-if="userStore.hasAnyPermission(['department:update', 'admin', 'hr', '*'])"
      ...
      >
        ...
        <el-table-column label="操作" width="230" fixed="right">
          <template #default="{ row }">
            ...
            <el-button 
              v-if="userStore.hasAnyPermission(['department:delete', 'admin', '*'])"
              type="danger" 
              size="small" 
...
const getDepartmentList = async () => {
  ...
  try {
    ...
    pagination.totalPages = Math.ceil(data.total / data.pageSize)
  } catch (error) {
...
const showCreateDialog = () => {
  if (!userStore.hasAnyPermission(['department:create', 'admin', 'hr', '*'])) {
    ElMessage.error('您没有创建部门的权限')
    return
  }
...
const handleDetailEdit = (department: Department) => {
  if (!userStore.hasAnyPermission(['department:update', 'admin', 'hr', '*'])) {
    ElMessage.error('您没有编辑部门的权限')
    return
  }
...
const handleDelete = async (department: any) => {
  if (!userStore.hasAnyPermission(['department:delete', 'admin', '*'])) {
    ElMessage.error('您没有删除部门的权限')
    return
  }
...
// 导出数据
const handleExport = () => {
  if (!userStore.hasAnyPermission(['department:export', 'admin', 'hr', '*'])) {
...
.batch-actions :deep(.el-card__body) {
  display: flex;
