...
const simulationParams = reactive({
  bonusPoolId: null as number | string | null,
  totalProfit: 10000000,
  poolRatio: 0.15,
  lineWeights: {} as Record<string | number, number>,
  minBonusRatio: 0.8,
  maxBonusRatio: 3.0
})
...
const scenarioForm = reactive({
  name: '',
  description: '',
  basePoolId: null as number | string | null,
  isPublic: false
})
...
const getWeightValidationClass = () => {
  ...
  if (Math.abs(parseFloat(total) - 100) < 1) return 'weight-valid'
...
const loadBusinessLines = async () => {
  try {
    ...
    // 过滤有效业务线并设置到组件状态
    businessLines.value = lines.filter((line: { id?: string; status: number }) => line && line.id && line.status === 1)
    console.log('✅ 成功加载业务线列表:', businessLines.value.length, '个业务线')
    console.log('📋 业务线详情:', businessLines.value.map(l => ({ id: l.id, name: l.name, code: l.code })))
...
// 根据业务线代码获取默认权重
const getDefaultWeightByCode = (code: string) => {
...
const refreshData = async () => {
  ...
  try {
    ...
    // 确保奖金池ID类型一致
    bonusPools.value = bonusPools.value.map(pool => {
      // 如果ID是字符串且可以转换为数字，则转换为数字
      if (typeof pool.id === 'string' && !isNaN(Number(pool.id))) {
        pool.id = Number(pool.id);
      }
      return pool;
    });
    
    console.log('加载的奖金池数据:', bonusPools.value.map(p => ({ 
      id: p.id, 
      type: typeof p.id, 
      period: p.period 
    })));
...
const handlePoolChange = () => {
  console.log('奖金池选择变化:', {
    selectedId: simulationParams.bonusPoolId,
    selectedType: typeof simulationParams.bonusPoolId,
    bonusPools: bonusPools.value.map(p => ({ id: p.id, type: typeof p.id, period: p.period }))
  });
  
  // 确保bonusPoolId的类型匹配
  const poolId = simulationParams.bonusPoolId;
  
  // 使用宽松比较处理数字和字符串类型
  currentPool.value = bonusPools.value.find(pool => {
    if (typeof pool._id === 'string' && typeof poolId === 'number') {
      return Number(pool._id) === poolId;
    } else if (typeof pool._id === 'number' && typeof poolId === 'string') {
      return pool._id === Number(poolId);
    } else {
      return pool._id == poolId; // 使用宽松比较
    }
  });
  if (currentPool.value) {
    ...
    console.log('找到匹配的奖金池:', currentPool.value);
  } else {
    console.log('未找到匹配的奖金池');
  }
}
...
const runSimulation = async () => {
  ...
  try {
    const requestData = {
      bonusPoolId: simulationParams.bonusPoolId as number,
...
const formatComparisonValue = (value: number, type: string) => {
  if (type === 'currency') {
...
const getComparisonValueClass = (value: number, type: string) => {
  // 根据值的大小返回不同的样式类
  return ''
}
...
const createScenario = async () => {
  ...
  try {
    await saveSimulationScenario({
      ...
      basePoolId: scenarioForm.basePoolId ? (scenarioForm.basePoolId as number) : undefined,
      ...
      series: [
        {
          ...
          data: businessLines.value.map(line => {
            ...
            return simulationResult.value?.lineComparison?.find((item: any) => item.lineId === lineId)?.currentBonus || 0
          })
        },
        {
          name: '模拟',
          ...
          data: businessLines.value.map(line => {
            const lineId = line._id || line.id
            return simulationResult.value?.lineComparison?.find((item: any) => item.lineId === lineId)?.simulatedBonus || 0
          })
        }
      ]
    })
    ...
    chart.setOption({
      ...
      tooltip: {
        ...
        formatter: (params: any) => {
          ...
          const changeRatio = parseFloat((point.data.changeRatio * 100).toFixed(1))
          const impact = parseFloat(((point.data.impact - 1) * 100).toFixed(1))
          return `变化: ${changeRatio > 0 ? '+' : ''}${changeRatio}%<br/>影响: ${impact > 0 ? '+' : ''}${impact}%`
        }
      },
      xAxis: {
        type: 'category',
        data: data.map((d: any) => `${(d.changeRatio * 100).toFixed(0)}%`),
        name: '参数变化',
        nameLocation: 'middle',
        nameGap: 30
      },
  ...
  if (historyChart.value && historyData.value) {
    ...
    chart.setOption({
      ...
      yAxis: { 
        ...
        nameGap: 50,
        axisLabel: {
          formatter: (value: any) => formatChartValue(value, data.metric)
        }
      },
      series: [{
        type: 'line',
