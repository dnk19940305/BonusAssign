  ...
  <div class="project-member-approval">
    <div class="page-header">
      ...
      <div class="header-actions">
        <el-button type="success" @click="showBatchApproveDialog" :disabled="selectedIds.length === 0">
    ...
    <div class="table-section">
      ...
      >
        ...
        <vxe-column field="expectedRoleName" title="期望角色" width="150">
          <template #default="{ row }">
        ...
        <vxe-column title="当前参与度" width="120" align="center">
          <template #default="{ row }">
            {{ (row.participationRatio > 1 ? row.participationRatio : row.participationRatio * 100).toFixed(0) }}%
          </template>
        </vxe-column>

        <vxe-column field="applyReason" title="申请理由" min-width="200" show-overflow="tooltip" />
        ...
        <vxe-column field="appliedAt" title="申请时间" width="160">
          <template #default="{ row }">
            {{ formatDate(row.appliedAt || row.submittedAt) }}
          </template>
        </vxe-column>
    ...
    >
      <div>
        <p>将要审批 <strong>{{ selectedIds.length }}</strong> 个申请</p>
        
        <el-alert
          title="提示：当前为不同角色分配模式，每行可设置不同的角色和参与度"
          type="info"
          :closable="false"
        ...
        >
          <el-form-item label="审批备注">
            <el-input
              v-model="batchApprovalForm.remark"
              type="textarea"
              :rows="3"
      ...
      <template #footer>
        <span class="dialog-footer">
          ...
          <el-button type="primary" @click="executeBatchApprove" :loading="submitting">
            确认批量审批
          </el-button>
        </span>
      </template>
...
const batchApprovalFormRef = ref()
const batchApprovalForm = reactive({
  remark: '',
  memberDetails: [] // 用于存储每个成员的详细信息
})

const showBatchApproveDialog = async () => {
  ...
  batchApprovalForm.memberDetails = []
  
  // 预加载成员详情
  await loadSelectedMemberDetails()
  
  batchApprovalDialogVisible.value = true
}

const loadSelectedMemberDetails = async () => {
  // 初始化成员详情列表，使用申请数据中的角色信息作为默认值
  batchApprovalForm.memberDetails = selectedIds.value.map(id => {
    const application = applicationList.value.find(app => app._id === id)
    return {
      memberId: id,
      employeeName: application?.employeeName || '未知员工',
      projectName: application?.projectName || '未知项目',
      expectedRoleName: application?.expectedRoleName || application?.roleName || '未指定',
      roleId: application?.roleId || '', // 使用申请数据中的角色ID作为默认值
      participationRatio: (application?.participationRatio || 1) * 100 // 转换为百分比
    }
  })
}
...
const executeBatchApprove = async () => {
  try {
    await ElMessageBox.confirm(
      `确定批量通过选中的 ${selectedIds.value.length} 个申请吗？`,
      '确认批量审批',
      {
        type: 'warning',
    ...
    // 为每个成员发送单独的配置
    const data = {
      memberApprovals: batchApprovalForm.memberDetails.map(detail => ({
        memberId: detail.memberId,
        roleId: detail.roleId,
        participationRatio: detail.participationRatio
      })),
      approved: true,
      remark: batchApprovalForm.remark
    }
...
const batchApprove = async () => {
  try {
    ...
    const data = {
      ...
      approved: true,
      defaultRoleId: validRoles.value[0]?._id || validRoles.value[0]?.id, // 使用第一个有效角色作为默认角色
      participationRatio: 100
    }
  ...
  try {
    ...
    if (approvalForm.action === 'approve') {
      ...
      await projectMemberApi.approveApplication(currentApplication.value._id, {
        ...
        participationRatio: approvalForm.participationRatio,
...
const manageRoles = (row) => {
  ...
  const ratio = row.participationRatio || 1
  roleForm.participationRatio = ratio > 1 ? Math.round(ratio) : Math.round(ratio * 100)
...
const updateMemberRole = async () => {
  try {
    ...
    await projectMemberApi.updateMemberRole(currentMember.value._id, {
      ...
      participationRatio: roleForm.participationRatio
    })
