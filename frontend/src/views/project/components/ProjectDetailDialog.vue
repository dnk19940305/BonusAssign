  ...
  >
    <div v-loading="loading" class="project-detail">
      <el-row :gutter="20" v-if="project">
        ...
        <!-- 财务概览 -->
        <el-col :span="24">
          <el-card class="detail-card finance-overview-card">
            <template #header>
              <div class="card-header">
                <span>财务概览</span>
              </div>
            ...
            <el-row :gutter="20">
              <el-col :span="12">
                <div class="detail-item">
                  <label>项目预算：</label>
                  <span v-if="project.budget" class="finance-amount budget-amount">{{ formatCurrency(project.budget) }}</span>
                  <span v-else class="text-muted">未设置</span>
                </div>
              </el-col>
              <el-col :span="12">
                <div class="detail-item">
                  <label>项目成本：</label>
                  <span v-if="(project as any).cost" :class="['finance-amount', 'cost-amount', getFinanceClass('cost', (project as any).cost)]">{{ formatCurrency((project as any).cost) }}</span>
            ...
            <el-row :gutter="20">
              <el-col :span="12">
                <div class="detail-item">
                  <label>利润目标：</label>
                  <span v-if="project.profitTarget" class="finance-amount target-amount">{{ formatCurrency(project.profitTarget) }}</span>
                  <span v-else class="text-muted">未设置</span>
                </div>
              </el-col>
              <el-col :span="12">
                <div class="detail-item">
                  <label>预期利润：</label>
                  <span :class="['finance-amount', 'profit-amount', getFinanceClass('profit', (project as any).expectedProfit || 0)]">{{ formatCurrency((project as any).expectedProfit || 0) }}</span>
                </div>
              </el-col>
            </el-row>
            ...
            <el-row :gutter="20">
              <el-col :span="12">
                <div class="detail-item">
                  <label>预估奖金：</label>
                  <span :class="['finance-amount', 'bonus-amount', getFinanceClass('bonus', (project as any).estimatedBonus || 0)]">{{ formatCurrency((project as any).estimatedBonus || 0) }}</span>
                </div>
              </el-col>
              <el-col :span="12">
                <div class="detail-item">
                  <label>已计算奖金：</label>
                  <span :class="['finance-amount', 'calculated-amount', getFinanceClass('calculated', (project as any).calculatedBonus || 0)]">{{ formatCurrency((project as any).calculatedBonus || 0) }}</span>
                </div>
              </el-col>
            </el-row>
          </el-card>
        ...
        <!-- 协作信息 -->
        <el-col :span="24">
          <el-card class="detail-card collaboration-card">
            <template #header>
              <div class="card-header">
                <span>协作信息</span>
              </div>
            ...
            <el-row :gutter="20">
              <el-col :span="8">
                <div class="detail-item">
                  ...
                  <span>
                    <el-tag 
                      :type="PROJECT_COOPERATION_STATUS_COLORS[project.cooperationStatus || 'draft']" 
                      size="small"
                    >
                      {{ PROJECT_COOPERATION_STATUS_LABELS[project.cooperationStatus || 'draft'] }}
                    </el-tag>
                  </span>
                </div>
              </el-col>
              ...
              <el-col :span="8">
                <div class="detail-item">
                  <label>已批准数量：</label>
                  <span class="approved-count">{{ project.approvedApplicationCount || 0 }}</span>
                </div>
              </el-col>
            </el-row>
          </el-card>
...
import { formatDate as formatDateUtil, formatCurrency as formatCurrencyUtil, formatPercentage, formatDateTime as formatDateTimeUtil } from '@/utils/format'
import ProjectWeightDialog from './ProjectWeightDialog.vue'
import type { Project, WeightConfig, ProjectWithCollaboration } from '@/types/project'
import { 
...
interface Props {
  modelValue: boolean
  project?: ProjectWithCollaboration | null
}
...
const getProjectProgress = (): number => {
  ...
  if (props.project?.overallProgress !== undefined && props.project?.overallProgress !== null) {
    return props.project.overallProgress
  }
...
// 获取财务数据颜色类
const getFinanceClass = (type: string, value: number) => {
  if (type === 'cost') {
    return 'cost-value'
  } else if (value > 0) {
...
.finance-overview-card .detail-item label {
  font-weight: 600;
  color: #303133;
}
...
.collaboration-card .detail-item label {
  font-weight: 600;
  color: #303133;
}
...
.profit-amount.positive-value {
  color: #67C23A !important;
}
  
.profit-amount.negative-value {
  color: #F56C6C !important;
}
  
.profit-amount.zero-value {
