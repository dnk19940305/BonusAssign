...
const handleSubmitApproval = async () => {
  try {
    ...
    const isApprove = approvalForm.value.decision === 'approve'

    // 2. 二次确认 - 详细信息展示
    const confirmTitle = isApprove ? '确认批准申请' : '确认拒绝申请'
    const confirmType = isApprove ? 'warning' : 'error'

    // 构建确认消息
    let confirmMessage = `
      <div style="text-align: left; line-height: 1.8;">
        <p style="font-size: 15px; margin-bottom: 12px;">
          <strong>项目：</strong>${props.application.project?.name || '未知项目'}
        </p>
        <p style="font-size: 14px; margin-bottom: 10px;">
          <strong>申请人：</strong>${props.application.applicantName || '未知'}
        </p>
        <p style="font-size: 14px; margin-bottom: 10px;">
          <strong>团队名称：</strong>${props.application.teamName || '未设置'}
        </p>
        <p style="font-size: 14px; margin-bottom: 10px;">
          <strong>团队规模：</strong>${props.application.teamSize || props.application.teamMembers?.length || 0} 人
        </p>
        <p style="font-size: 14px; margin-bottom: 10px;">
          <strong>预估成本：</strong>${formatCurrency(props.application.estimatedCost)}
        </p>
    `

    if (isApprove) {
      confirmMessage += `
        <div style="margin-top: 15px; padding: 12px; background: #f0f9ff; border-left: 3px solid #409EFF; border-radius: 4px;">
          <p style="color: #409EFF; margin: 0 0 8px 0;"><strong>✓ 批准后将执行以下操作：</strong></p>
          <ul style="margin: 0; padding-left: 20px; color: #606266;">
            <li>申请状态更新为"已批准"</li>
            <li>团队成员状态激活为"active"</li>
            <li>项目协作状态更新为"approved"</li>
            <li>${approvalForm.value.notifications.includes('notifyApplicant') ? '发送通知给申请人' : ''}</li>
            <li>${approvalForm.value.notifications.includes('notifyTeamMembers') ? '发送通知给团队成员' : ''}</li>
          </ul>
        </div>
      `
    } else {
      confirmMessage += `
        <div style="margin-top: 15px; padding: 12px; background: #fef0f0; border-left: 3px solid #F56C6C; border-radius: 4px;">
          <p style="color: #F56C6C; margin: 0 0 8px 0;"><strong>✗ 拒绝后将执行以下操作：</strong></p>
          <ul style="margin: 0; padding-left: 20px; color: #606266;">
            <li>申请状态更新为"已拒绝"</li>
            <li>删除待审批的成员记录</li>
            <li>项目状态回退为"published"(重新开放申请)</li>
            <li>发送拒绝通知给申请人</li>
          </ul>
          <p style="margin: 10px 0 0 0; color: #909399; font-size: 13px;">
            <strong>拒绝理由：</strong>${approvalForm.value.comments}
          </p>
        </div>
      `
    }
    ...
    confirmMessage += `
      ...
      <p style="margin-top: 15px; font-size: 13px; color: #909399; text-align: center;">
        此操作将立即生效且<strong>不可撤销</strong>,请仔细确认!
      </p>
    `
    ...
    await ElMessageBox.confirm(
      ...
      confirmTitle,
      {
        confirmButtonText: isApprove ? '确定批准' : '确定拒绝',
        cancelButtonText: '取消',
        type: confirmType,
        dangerouslyUseHTMLString: true,
        distinguishCancelAndClose: true,
        customClass: 'approval-confirm-box'
      }
    )
