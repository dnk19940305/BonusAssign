      ...
      <el-table :data="members" style="width: 100%">
        ...
        <el-table-column prop="estimatedWorkload" label="工作量占比" width="120" align="center">
          <template #default="{ row }">
            {{ row.estimatedWorkload > 1 ? Math.round(row.estimatedWorkload) : Math.round(row.estimatedWorkload * 100) }}%
          </template>
        </el-table-column>
...
const loadTeamMembers = async () => {
  ...
  try {
    // 直接从application中获取成员信息（后端返回的字段名是Members）
    if (props.application.Members && Array.isArray(props.application.Members)) {
      members.value = props.application.Members
    } else if (props.application.members && Array.isArray(props.application.members)) {
      // 兼容小写的members字段
      members.value = props.application.members
    } else {
      members.value = []
      console.warn('申请数据中没有成员信息', props.application)
    }
  } catch (error) {
