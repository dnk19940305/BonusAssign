      ...
      >
        ...
        <el-table-column label="部门" width="120">
          <template #default="{ row }">
            {{ row.Department?.name || row.department?.name || row.departmentName || '-' }}
          </template>
        </el-table-column>
        <el-table-column label="岗位" width="120">
          <template #default="{ row }">
            {{ row.Position?.name || row.position?.name || row.positionName || '-' }}
          </template>
        </el-table-column>
        <el-table-column label="级别" width="80">
          <template #default="{ row }">
            {{ row.Position?.level || row.position?.level || row.level || '-' }}
          </template>
        </el-table-column>
...
const handleConfirm = () => {
  ...
  const teamMembers: TeamMember[] = selectedEmployees.value.map(employee => ({
    ...
    Employee: {
      ...
      // 兼容大小写两种情况
      department: employee.Department?.name || employee.department?.name || employee.departmentName,
      position: employee.Position?.name || employee.position?.name || employee.positionName
    }
  }))
