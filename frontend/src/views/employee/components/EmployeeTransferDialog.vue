      ...
      <el-card class="current-info">
        ...
        <el-row :gutter="20">
          ...
          <el-col :span="12">
            <div class="info-item">
              <label>当前部门：</label>
              <span>{{ employee.department?.name || employee.Department?.name || '-' }}</span>
            </div>
          </el-col>
        </el-row>
        ...
        <el-row :gutter="20">
          <el-col :span="12">
            <div class="info-item">
              <label>当前岗位：</label>
              <span>{{ employee.position?.name || employee.Position?.name || '-' }}</span>
            </div>
          </el-col>
          <el-col :span="12">
            <div class="info-item">
      ...
      <el-card class="change-preview" v-if="hasChanges">
        ...
        <div class="change-list">
          <div v-if="departmentChange" class="change-item">
            ...
            <span>部门：{{ employee.department?.name || employee.Department?.name }} → {{ getTargetDepartmentName() }}</span>
          </div>
          
          <div v-if="positionChange" class="change-item">
            <el-icon><Right /></el-icon>
            <span>岗位：{{ employee.position?.name || employee.Position?.name }} → {{ getTargetPositionName() }}</span>
          </div>
