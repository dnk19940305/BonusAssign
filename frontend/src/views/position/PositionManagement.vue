<template>
  <div class="position-management">
    ...
    <el-card class="search-card">
      <el-form :model="queryForm" inline>
        ...
        <el-form-item label="职级">
          <el-select v-model="queryForm.level" placeholder="请选择职级" clearable style="width: 150px;">
            <el-option
    ...
    <el-card 
      class="batch-actions" 
      v-if="selectedPositions.length > 0 && userStore.hasAnyPermission(['position:update', 'admin', 'hr', '*'])"
    >
      <el-alert
      ...
      <div class="batch-buttons">
        ...
        <el-button 
          v-if="userStore.hasAnyPermission(['position:update', 'admin', 'hr', '*'])"
          @click="showBatchBenchmarkDialog"
        >
          批量调整基准值
        </el-button>
      </div>
        ...
        <el-table-column label="操作" width="350" fixed="right">
          <template #default="{ row }">
            ...
            <el-button 
              v-if="userStore.hasAnyPermission(['position:update', 'admin', 'hr', '*'])"
              size="small" 
              @click="adjustBenchmark(row)"
            >
              调整基准值
            </el-button>
            <el-button 
              v-if="userStore.hasAnyPermission(['position:delete', 'admin', '*'])"
              type="danger" 
              size="small" 
...
const toggleStatus = async (position: Position) => {
  try {
    const action = position.status === 0 ? '启用' : '禁用'
    const actionVerb = position.status === 0 ? 'enable' : 'disable'
    
    await ElMessageBox.confirm(
      `确定要${action}岗位 ${position.name} 吗？`,
      `确认${action}`,
      {
    ...
    await updatePositionStatus(position.id!, position.status === 0 ? 1 : 0)
