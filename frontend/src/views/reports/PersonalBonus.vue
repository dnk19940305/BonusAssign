    ...
    <el-card v-if="bonusInfo" class="result-card">
      ...
      <div class="bonus-overview">
        <el-row :gutter="20">
          ...
          <el-col :span="8">
            <div class="overview-card regular">
              <div class="overview-icon">
                <el-icon><Wallet /></el-icon>
              </div>
              <div class="overview-content">
                <div class="overview-value">¥{{ formatNumber(bonusInfo.regularBonus?.totalAmount || 0) }}</div>
                <div class="overview-label">常规奖金（三维）</div>
              </div>
            </div>
          </el-col>
          <el-col :span="8">
            <div class="overview-card project">
              <div class="overview-icon">
                <el-icon><TrendCharts /></el-icon>
              </div>
              <div class="overview-content">
                <div class="overview-value">¥{{ formatNumber(bonusInfo.projectBonus?.totalAmount || 0) }}</div>
                <div class="overview-label">项目奖金</div>
              </div>
            </div>
          </el-col>
        </el-row>
      ...
      <!-- 三维奖金细分（仅展示常规奖金的构成） -->
      <el-card v-if="bonusInfo.bonusBreakdown" class="breakdown-detail" style="margin-top: 20px;">
        <template #header>
          <span>常规奖金构成（三维模型）</span>
        </template>
        <el-row :gutter="20">
          <el-col :span="8">
            <div class="breakdown-item">
              <div class="breakdown-label">利润贡献</div>
              <div class="breakdown-value">¥{{ formatNumber(bonusInfo.bonusBreakdown.profitContribution) }}</div>
              <div class="breakdown-percentage">
                {{ calculatePercentage(bonusInfo.bonusBreakdown.profitContribution, bonusInfo.regularBonus?.totalAmount) }}%
              </div>
            </div>
          </el-col>
          <el-col :span="8">
            <div class="breakdown-item">
              <div class="breakdown-label">岗位价值</div>
              <div class="breakdown-value">¥{{ formatNumber(bonusInfo.bonusBreakdown.positionValue) }}</div>
              <div class="breakdown-percentage">
                {{ calculatePercentage(bonusInfo.bonusBreakdown.positionValue, bonusInfo.regularBonus?.totalAmount) }}%
              </div>
            </div>
          </el-col>
          <el-col :span="8">
            <div class="breakdown-item">
              <div class="breakdown-label">绩效表现</div>
              <div class="breakdown-value">¥{{ formatNumber(bonusInfo.bonusBreakdown.performance) }}</div>
              <div class="breakdown-percentage">
                {{ calculatePercentage(bonusInfo.bonusBreakdown.performance, bonusInfo.regularBonus?.totalAmount) }}%
              </div>
            </div>
          </el-col>
        </el-row>
...
// 计算百分比
const calculatePercentage = (part: number, total: number | undefined) => {
...
const handleSearch = async () => {
  ...
  try {
    ...
    bonusInfo.value = {
      ...
      // ✅ 使用正确的字段名称
      regularBonus: overviewData.bonusData?.regularBonus || null,
      projectBonus: overviewData.bonusData?.projectBonus || null,
      bonusBreakdown: overviewData.bonusData?.bonusBreakdown || null,
