    ...
    >
      ...
      <el-form-item label="KPI指标">
        <div class="kpi-metrics">
          <div v-if="formData.kpi_metrics.length === 0" class="no-kpi">
          ...
          <div v-else>
            ...
            <div v-for="(metric, index) in formData.kpi_metrics" :key="index" class="kpi-item">
              <el-row :gutter="12" align="middle">
                <el-col :span="8">
                  <el-input
                    v-model="metric.name"
...
const initFormData = () => {
  if (props.mode === 'edit' && props.businessLine) {
    ...
    formData.kpi_metrics = props.businessLine.kpi_metrics ? [...props.businessLine.kpi_metrics] : []
  } else {
    formData.name = ''
    formData.code = ''
    ...
    formData.kpi_metrics = []
  }
}

// 添加KPI指标
const addKpiMetric = () => {
  formData.kpi_metrics.push({
    name: '',
    target: 0,
    weight: 0
  })
...
const removeKpiMetric = (index: number) => {
  formData.kpi_metrics.splice(index, 1)
}
...
const getEmployees = async () => {
  try {
    ...
    employees.value = response.data.list.map(emp => ({
      id: emp.id,
      name: emp.name,
...
const handleSubmit = async () => {
  ...
  try {
    ...
    // 验证KPI权重总和
    if (formData.kpi_metrics.length > 0) {
      const totalWeight = formData.kpi_metrics.reduce((sum, metric) => sum + metric.weight, 0)
      ...
      // 验证KPI指标名称不能为空
      const hasEmptyName = formData.kpi_metrics.some(metric => !metric.name.trim())
      if (hasEmptyName) {
    ...
    const submitData: BusinessLineCreateData | BusinessLineUpdateData = {
      ...
      kpiMetrics: formData.kpi_metrics.length > 0 ? formData.kpi_metrics :  []
    }
