...
import skillTagApi from '@/api/skillTag'
import type { SkillTag, SkillCategory, SkillTagForm, SkillCategoryForm } from '@/api/skillTag'
...
const refreshData = async () => {
  ...
  try {
    const [tagsResponse, categoriesResponse] = await Promise.all([
      skillTagApi.getSkillTags({ page: pagination.page, pageSize: pagination.pageSize }),
      skillTagApi.getSkillCategories()
    ])
    
    skillTags.value = tagsResponse.data.tags
    pagination.total = tagsResponse.data.pagination.total
    skillCategories.value = categoriesResponse.data
    
    ElMessage.success('数据刷新成功')
  } catch (error: any) {
    console.error('数据刷新失败:', error)
    ElMessage.error(error.response?.data?.message || '数据刷新失败')
  } finally {
    loading.value = false
  }
}
...
const submitTagForm = async () => {
  try {
    const formData: SkillTagForm = {
      name: tagForm.name,
      code: tagForm.code,
      categoryId: tagForm.category,
      level: tagForm.level,
      description: tagForm.description,
      synonyms: tagForm.synonyms,
      relatedSkills: tagForm.relatedSkills,
      isActive: true
    }
    
    if (isEditTag.value) {
      await skillTagApi.updateSkillTag(tagForm.id, formData)
    } else {
      await skillTagApi.createSkillTag(formData)
    }
    ...
    ElMessage.error(error.response?.data?.message || (isEditTag.value ? '更新标签失败' : '创建标签失败'))
  }
}

const toggleTagStatus = async (tag: SkillTag) => {
  try {
    await skillTagApi.updateSkillTag(tag.id, { isActive: !tag.isActive })
...
const submitCategoryForm = async () => {
  try {
    const formData: SkillCategoryForm = {
    ...
    if (isEditCategory.value) {
      await skillTagApi.updateSkillCategory(categoryForm.id, formData)
    } else {
      await skillTagApi.createSkillCategory(formData)
    }
    ...
    ElMessage.error(error.response?.data?.message || (isEditCategory.value ? '更新分类失败' : '创建分类失败'))
  }
}
...
const deleteCategory = async (category: SkillCategory) => {
  ...
  try {
    ...
    await skillTagApi.deleteSkillCategory(category.id)
...
const handleBatchEnable = async () => {
  try {
    ...
    await skillTagApi.batchOperateSkillTags(ids, 'enable')
...
const handleBatchDisable = async () => {
  try {
    const ids = selectedTags.value.map(tag => tag.id)
    await skillTagApi.batchOperateSkillTags(ids, 'disable')
...
const handleBatchDelete = async () => {
  try {
    ...
    const ids = selectedTags.value.map(tag => tag.id)
    await skillTagApi.batchOperateSkillTags(ids, 'delete')
...
const confirmMerge = async () => {
  ...
  try {
    ...
    await skillTagApi.mergeSkillTags(
      mergeForm.primaryTagId,
      secondaryTag.id,
      mergeForm.description
    )
    ...
    ElMessage.error(error.response?.data?.message || '标签合并失败')
  }
}
