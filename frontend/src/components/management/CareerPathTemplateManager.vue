...
import careerPathTemplateApi from '@/api/careerPathTemplate'
import type { CareerPathTemplate, CareerPathTemplateForm } from '@/api/careerPathTemplate'
...
const refreshData = async () => {
  loading.value = true
  try {
    const response = await careerPathTemplateApi.getCareerPathTemplates({
      page: 1,
      pageSize: 100
    })
    careerPathTemplates.value = response.data.templates
...
const copyTemplate = async (template: CareerPathTemplate) => {
  try {
    await careerPathTemplateApi.copyCareerPathTemplate(template.id)
    
    ElMessage.success('模板复制成功')
    await refreshData()
  } catch (error: any) {
    console.error('模板复制失败:', error)
    ElMessage.error(error.response?.data?.message || '模板复制失败')
  }
}
...
const deleteTemplate = async (template: CareerPathTemplate) => {
  try {
    ...
    await careerPathTemplateApi.deleteCareerPathTemplate(template.id)
...
const submitTemplateForm = async () => {
  try {
    const formData: CareerPathTemplateForm = {
      ...
      careerPath: templateForm.careerPath,
      skillDevelopment: templateForm.skillDevelopment
    }
    
    if (isEditTemplate.value) {
      await careerPathTemplateApi.updateCareerPathTemplate(templateForm.id, formData)
    } else {
      await careerPathTemplateApi.createCareerPathTemplate(formData)
    }
    ...
    ElMessage.error(error.response?.data?.message || (isEditTemplate.value ? '更新模板失败' : '创建模板失败'))
  }
}
