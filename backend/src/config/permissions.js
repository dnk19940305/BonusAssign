...
const PERMISSIONS = {
  ...
  // 项目成员管理权限
  PROJECT_MEMBER: {
    VIEW: 'project_member:view',
    CREATE: 'project_member:create',
    UPDATE: 'project_member:update',
    DELETE: 'project_member:delete',
    ALL: 'project_member:*'
  },
  ...
  // 改进建议权限
  IMPROVEMENT: {
    VIEW: 'improvement:view',
    CREATE: 'improvement:create',
    EDIT: 'improvement:edit',
...
const ROLE_PERMISSIONS = {
  ...
  HR_ADMIN: [
    ...
    PERMISSIONS.IMPROVEMENT.EDIT,
    PERMISSIONS.IMPROVEMENT.DELETE
  ],
  ...
  PROJECT_MANAGER: [
    ...
    PERMISSIONS.PROJECT.COST_VIEW,  // 添加项目成本查看权限
    PERMISSIONS.PROJECT.COST_CREATE,  // 添加项目成本创建权限
    PERMISSIONS.PROJECT.COST_UPDATE,  // 添加项目成本更新权限
    PERMISSIONS.PROJECT.COST_DELETE,  // 添加项目成本删除权限
  ...
  EMPLOYEE: [
    ...
    PERMISSIONS.REPORT.VIEW,
    PERMISSIONS.IMPROVEMENT.CREATE,
    PERMISSIONS.IMPROVEMENT.VIEW
  ]
}
...
class PermissionValidator {
  ...
  static checkProjectPermission(user, action, project) {
    ...
    // 财务人员和HR可以查看所有项目（用于成本管理和奖金计算）
    if (action === 'view' && this.hasPermission(userPermissions, [
    ...
    // 项目经理可以管理自己管理的项目
    if (action === 'manage' && project.managerId === (user.employeeId || user.id)) {
      return true
    }

    // 项目经理可以查看自己管理的项目
    if (action === 'view' && project.managerId === (user.employeeId || user.id)) {
      return true
    }

    // 项目成员可以查看自己参与的项目
    if (action === 'view' && project.members && project.members.includes(user.employeeId || user.id)) {
      return true
    }
