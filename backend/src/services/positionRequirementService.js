...
class PositionRequirementService {
  ...
  async createPositionRequirement(requirementData) {
    try {
      ...
      const result = await databaseService.insert('positionRequirements', requirement)
  ...
  async getPositionRequirements(query = {}, options = {}) {
    try {
      ...
      if (search) {
        // 由于databaseService的find方法可能不直接支持MySQL的LIKE查询，
        // 我们先获取所有记录然后在内存中过滤
        // 在实际应用中，可能需要databaseService支持原生SQL查询
      }

      // 获取岗位要求列表
      // 由于需要支持搜索功能，我们先获取所有匹配基本条件的记录
      const allRequirements = await databaseService.find('positionRequirements', filterQuery, {
        sort: { updatedAt: -1 }
      })
      ...
      if (search) {
        requirements = allRequirements.filter(req => {
          const searchLower = search.toLowerCase()
          return (
            (req.positionName && req.positionName.toLowerCase().includes(searchLower)) ||
            (req.positionCode && req.positionCode.toLowerCase().includes(searchLower))
          )
        })
      } else {
  ...
  async getPositionRequirementByPositionId(positionId) {
    try {
      const requirement = await databaseService.findOne('positionRequirements', { 
  ...
  async updatePositionRequirement(id, updateData) {
    try {
      ...
      await databaseService.update('positionRequirements', 
        { _id: id }, 
        updateFields
      )

      // 获取更新后的记录
      const updatedRequirement = await databaseService.findOne('positionRequirements', { 
        _id: id 
      })
  ...
  async deletePositionRequirement(id) {
    try {
      const result = await databaseService.remove('positionRequirements', { 
        _id: id 
      })
  ...
  async getApprovalStats() {
    try {
      const allRequirements = await databaseService.find('positionRequirements', {})
  ...
  async batchOperation(ids, action, updateData = {}) {
    try {
      ...
      // 使用原生MySQL查询方式，通过多次查询后合并
      let updateCount = 0
      for (const id of ids) {
        const updateResult = await databaseService.update('positionRequirements', id, updateFields)
