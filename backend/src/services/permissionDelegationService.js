...
class PermissionDelegationService {
  ...
  async createDelegation(delegator, delegatee, permissions, options = {}) {
    try {
      ...
      // 保存到数据库
      const savedDelegation = await databaseService.insert('permissionDelegations', delegation.toObject())
  ...
  async approveDelegation(delegationId, approver, approved, comments = '') {
    try {
      // 获取委派信息
      const delegation = await databaseService.findOne('permissionDelegations', { id: delegationId })
      if (!delegation) {
      ...
      if (approved) {
        ...
        await databaseService.update('permissionDelegations', { id: delegationId }, delegation.toObject())
  ...
  async cleanupExpiredDelegations() {
    try {
      const expiredDelegations = await databaseService.find('permissionDelegations', {
        status: 'active'
      })
      ...
      for (const delegation of expiredDelegations) {
        if (delegation.endTime && delegation.endTime < now) {
          ...
          await databaseService.update('permissionDelegations', { id: delegation.id }, delegation.toObject())
          cleanedCount++
        }
      }
