...
class NotificationService {
  ...
  async createNotification(notificationData) {
    try {
      ...
      const result = await databaseService.insert('notifications', notification)
  ...
  async createBatchNotifications(notificationsData) {
    try {
      ...
      const result = await databaseService.insert('notifications', notifications)
  ...
  async getUserNotifications(userId, options = {}) {
    try {
      ...
      // 获取通知列表
      const notifications = await databaseService.find('notifications', query, {
        sort: { createdAt: -1 }
      })
  ...
  async markNotificationRead(notificationId, userId) {
    try {
      // 检查通知是否存在且属于该用户
      const notification = await databaseService.findOne('notifications', { 
        _id: notificationId,
        userId 
      })
      ...
      // 更新通知状态
      await databaseService.update('notifications', 
        { _id: notificationId }, 
        { 
          isRead: true, 
          readAt: new Date(),
      ...
      // 获取更新后的通知
      const updatedNotification = await databaseService.findOne('notifications', { 
        _id: notificationId 
      })
  ...
  async markAllNotificationsRead(userId, filter = {}) {
    try {
      ...
      const result = await databaseService.update('notifications', 
        query, 
        { 
          isRead: true, 
          readAt: new Date(),
  ...
  async deleteNotification(notificationId, userId) {
    try {
      ...
      // 删除通知
      const result = await databaseService.remove('notifications', { 
        _id: notificationId 
      })
  ...
  async getNotificationStats(userId) {
    try {
      const allNotifications = await databaseService.find('notifications', { userId })
  ...
  /**
   * 发送项目发布通知
   * 通知符合技能要求的项目经理有新项目可申请
   * @param {string} projectId - 项目ID
   * @param {string} projectName - 项目名称
   * @param {string} projectCode - 项目代码
   * @param {Array} recipientIds - 接收人ID数组(潜在项目经理)
   * @param {Object} projectDetails - 项目详细信息
   * @returns {Array} 创建的通知记录
   */
  async notifyProjectPublished(projectId, projectName, projectCode, recipientIds, projectDetails = {}) {
    try {
      if (!recipientIds || recipientIds.length === 0) {
        logger.warn('📢 项目发布通知: 无接收人', { projectId, projectName })
        return []
      }
      ...
      const notifications = recipientIds.map(recipientId => ({
        ...
        title: '新项目发布通知',
        content: `项目"${projectName}"(${projectCode})已发布,欢迎申请团队组建。预算: ${projectDetails.budget || 'N/A'}`,
        relatedId: projectId,
        metadata: {
          projectId,
          projectName,
          ...
          bonusScale: projectDetails.bonusScale,
          priority: projectDetails.priority,
          skillRequirements: projectDetails.skillRequirements || [],
   ...
   * @param {Array} reviewerIds - 审批人ID数组(管理层)
   * @param {Object} applicationDetails - 申请详细信息
   * @returns {Array} 创建的通知记录
   */
  async notifyTeamApplicationSubmitted(
    applicationId,
    projectId,
    projectName,
    applicantId,
    ...
    try {
      if (!reviewerIds || reviewerIds.length === 0) {
        logger.warn('📋 团队申请通知: 无审批人', { applicationId, projectName })
        return []
      }
      ...
      const notifications = reviewerIds.map(recipientId => ({
        ...
        title: '团队申请待审批',
        content: `${applicantName}提交了项目"${projectName}"的团队申请(${teamName}),成员${applicationDetails.totalMembers || 0}人,预估成本${applicationDetails.estimatedCost || 'N/A'},请及时审批`,
        relatedId: applicationId,
        metadata: {
          applicationId,
          projectId,
          ...
          applicantName,
          teamName,
          totalMembers: applicationDetails.totalMembers,
          estimatedCost: applicationDetails.estimatedCost,
          applicationReason: applicationDetails.applicationReason,
      ...
      // TODO: 集成实时推送 (WebSocket / SSE)
      this._pushRealtime(reviewerIds, {
  ...
  async notifyApprovalResult(
    ...
    try {
      ...
      const notification = {
        userId: applicantId,
        projectId,
        notificationType: `team_application_${action}`,
        title: isApproved ? '✅ 团队申请已批准' : '❌ 团队申请被拒绝',
        content: isApproved
          ? `恭喜!您的项目"${projectName}"团队申请(${teamName})已获批准。审批人: ${approverName}。审批意见: ${comments || '无'}`
          : `抱歉,您的项目"${projectName}"团队申请(${teamName})被拒绝。审批人: ${approverName}。拒绝理由: ${comments || '未说明'}`,
        relatedId: applicationId,
        metadata: {
          applicationId,
          projectId,
      ...
      logger.info(isApproved ? '✅ 批准通知发送成功' : '❌ 拒绝通知发送成功', {
        applicationId,
        projectName,
        teamName,
        action,
      ...
      // TODO: 集成实时推送 (WebSocket / SSE)
      this._pushRealtime([applicantId], {
  ...
  _pushRealtime(recipientIds, payload) {
    ...
    //     const userSocket = global.webSocketServer.getUserSocket(userId)
    //     if (userSocket) {
