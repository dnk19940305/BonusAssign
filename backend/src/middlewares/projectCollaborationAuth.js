...
const checkTeamApplyPermission = async (req, res, next) => {
  try {
    ...
    if (projectId) {
      ...
      // 检查协作状态是否允许申请
      // 情况1：已发布（published）的项目可以申请
      // 情况2：未发布（draft）且未指定项目经理的项目也可以申请（抢单模式）
      const cooperationStatus = project.cooperationStatus || project.cooperation_status || null
      const managerId = project.managerId || project.manager_id || null
      
      const isPublished = cooperationStatus === 'published'
      const isDraftWithoutManager = (cooperationStatus === 'draft' || !cooperationStatus) && !managerId
      
      if (!isPublished && !isDraftWithoutManager) {
      ...
      const existingMember = members.find(m => m.employeeId === (user.employeeId || user.id))
      if (existingMember) {
        return res.status(400).json({
          code: 400,
          message: '您已经是项目成员',
...
const checkMemberApprovalPermission = async (req, res, next) => {
  try {
    ...
    // 检查项目级权限
    if (projectIdParam) {
      const project = await databaseService.getProjectById(projectIdParam)
      if (!project) {
        return res.status(404).json({
          code: 404,
          message: '项目不存在',
      ...
      // 针对审批路由: 根据申请ID反查项目并验证权限
      const application = await databaseService.getTeamApplicationById(applicationId)
      if (!application) {
        return res.status(404).json({
          code: 404,
          message: '申请不存在',
      ...
      const projId = application.projectId || application.project_id
      const project = await databaseService.getProjectById(projId)
      if (!project || !PermissionValidator.checkProjectPermission(user, 'manage', project)) {
        return res.status(403).json({
          code: 403,
          message: '您没有权限审批此项目的成员申请',
