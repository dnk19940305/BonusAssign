...
class OptimizationInstaller {
  ...
  async installOptimizations() {
    ...
    const optimizations = [
      ...
      {
        name: '性能监控中间件',
        source: 'middlewares/performanceMonitor.js',
        target: 'middlewares/performanceMonitor.js',
        backup: false
      }
    ];
  ...
  async updateAppConfig() {
    ...
    if (fs.existsSync(appPath)) {
      ...
      if (!appContent.includes('performanceMonitor')) {
        const middlewareImport = `const { middleware: performanceMiddleware } = require('./middlewares/performanceMonitor');\n`;
