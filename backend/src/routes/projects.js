 ...
 *     tags:
 *       - 项目管理
 *     summary: （已废弃）获取用户可申请的项目列表
 *     deprecated: true
 *     description: 该API已废弃，请使用 /api/project-collaboration/available-projects 代替。新API支持分页、搜索、权限控制等功能。
 *     security:
 *       - bearerAuth: []
 *     responses:
 *       200:
 ...
 *                 message:
 *                   type: string
 *                   example: "该API已废弃，请使用 /api/project-collaboration/available-projects"
 *       400:
 *         description: 用户未关联员工信息
 */
// 已废弃：请使用 /api/project-collaboration/available-projects
router.get('/available', 
  ...
  (req, res) => {
    res.json({
      code: 200,
      message: '该API已废弃，请使用 /api/project-collaboration/available-projects 代替。新API支持分页、搜索、权限控制等功能。',
      data: {
        deprecated: true,
        newEndpoint: '/api/project-collaboration/available-projects',
        features: ['支持分页', '支持搜索', '过滤已申请项目', '权限控制']
      }
    })
  }
)
 ...
 *         description: 更新成功
 *       403:
 *         description: 权限不足（只有管理员或项目经理可以修改）
 *       404:
 *         description: 项目不存在
 */
router.put('/:id', 
  // 只需要有project:view权限即可，具体权限判断由controller处理
  authorize(['project:view']),
 ...
 *         description: 提前结项成功
 *       400:
 *         description: 当前状态不允许提前结项
 *       404:
 *         description: 项目不存在
 */
router.post('/:id/complete', 
  authorize(['project:update']),
  logOperation('提前结项', '项目'),
