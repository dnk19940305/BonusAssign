...
const userSchemas = {
  ...
  update: Joi.object({
    ...
    password: Joi.string().pattern(/^(?=.*[A-Za-z])(?=.*\d)[A-Za-z\d@$!%*#?&]{8,}$/).allow('').optional()
      .messages({
        'string.pattern.base': '密码至少8位，需包含字母和数字'
      }),
...
const employeeSchemas = {
  ...
  update: Joi.object({
    employeeNo: Joi.string().max(50).optional(),
    name: Joi.string().min(2).max(50).required(),
    departmentId: Joi.string().required(),
    positionId: Joi.string().required(),
    annualSalary: Joi.number().positive().required(),
...
const citySchemas = {
  create: Joi.object({
    ...
    tier: Joi.string().min(1).max(20).required(),
    coefficient: Joi.number().min(0.1).max(2.0).required(),
  ...
  update: Joi.object({
    ...
    tier: Joi.string().min(1).max(20),
    coefficient: Joi.number().min(0.1).max(2.0),
    description: Joi.string().max(500).allow('', null).optional()
  })
}
