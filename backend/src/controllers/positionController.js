...
class PositionController {
  ...
  async getPositions(req, res, next) {
    try {
      ...
      // 获取所有岗位
      let positions = await databaseService.getPositions()
      // 搜索过滤
      if (search) {
      ...
      // 分类过滤
      if (categoryId) {
        positions = positions.filter(pos => pos.categoryId === categoryId)
      }
      ...
      const positionsWithDetails = await Promise.all(
        paginatedPositions.map(async (position) => {
          ...
          if (position.businessLineId) {
            businessLine = await databaseService.getBusinessLineById(position.businessLineId)
          }
  ...
  async getPosition(req, res, next) {
    try {
      ...
      const position = await databaseService.getPositionById(id)
  ...
  async createPosition(req, res, next) {
    try {
      ...
      // 检查代码是否已存在
      const existingPosition = await databaseService.findOne('positions', { 
      ...
      if (positionData.businessLineId) {
        const businessLine = await databaseService.getBusinessLineById(positionData.businessLineId)
        if (!businessLine) {
          return res.status(400).json({
            code: 400,
            message: '指定的业务线不存在',
      ...
      const newPosition = await databaseService.createPosition(positionData)

      // 移除_id，只使用id
      const { _id, ...newPosData } = newPosition
  ...
  // 更新岗位状态
  async updatePositionStatus(req, res, next) {
    try {
      const { id } = req.params
      const { status } = req.body

      // 检查岗位是否存在
      const existingPosition = await databaseService.getPositionById(id)
      if (!existingPosition) {
        return res.status(404).json({
          code: 404,
          message: '岗位不存在',
      ...
      const result = await databaseService.updatePosition(id, { status })

      if (result > 0) {
        const updatedPosition = await databaseService.getPositionById(id)
        // 移除_id，只使用id
        const { _id, ...positionData } = updatedPosition
      ...
      logger.error('Update position status error:', error)
      next(error)
    }
  }
  ...
  async updatePosition(req, res, next) {
    try {
      ...
      if (updateData.code && updateData.code !== existingPosition.code) {
        const codeExists = await databaseService.findOne('positions', { 
      ...
      if (updateData.businessLineId && updateData.businessLineId !== existingPosition.businessLineId) {
        const businessLine = await databaseService.getBusinessLineById(updateData.businessLineId)
        if (!businessLine) {
          return res.status(400).json({
            code: 400,
            message: '指定的业务线不存在',
      ...
      const result = await databaseService.updatePosition(id, updateData)
  ...
  async deletePosition(req, res, next) {
    try {
      ...
      const result = await databaseService.deletePosition(id)
  ...
  async getPositionStats(req, res, next) {
    try {
      ...
      const positions = await databaseService.find('positions', query)
  ...
  async batchPositions(req, res, next) {
    try {
      ...
      for (const positionId of positionIds) {
        try {
          const result = await databaseService.updatePosition(positionId, { status })
  ...
  async batchUpdateBenchmarkValues(req, res, next) {
    try {
      ...
      for (const pos of positions) {
        try {
          const result = await databaseService.updatePosition(pos.id, {
  ...
  async updatePositionRequirements(req, res, next) {
    try {
      ...
      const result = await databaseService.updatePosition(positionId, updateData)
  ...
  async updatePositionRequirementsDB(positionCode, requirements) {
    try {
      // 查找岗位要求记录
      const existingRequirement = await databaseService.findOne('position_requirements', { 
        positionCode 
      })

      if (existingRequirement) {
        // 更新现有记录
        await databaseService.update('position_requirements', 
        ...
        // 创建新记录
        await databaseService.insert('position_requirements', {
          positionCode,
  ...
  async createPosition(req, res, next) {
    try {
      ...
      if (!positionData.name || !positionData.code || !positionData.level || !positionData.benchmarkValue) {
        return res.status(400).json({
          code: 400,
          message: '缺少必填字段',
          data: null
      ...
      // 检查岗位代码是否已存在
      const existingPosition = await databaseService.findOne('positions', { code: positionData.code })
      if (existingPosition) {
        return res.status(400).json({
          code: 400,
          message: '岗位代码已存在',
      ...
      const result = await databaseService.insert('positions', newPosition)
  ...
  async deletePosition(req, res, next) {
    try {
      ...
      // 物理删除岗位
      await databaseService.remove('positions', { _id: id })
    ...
    try {
      ...
      for (const positionId of positionIds) {
        try {
          ...
          await databaseService.update('positions', 
            { _id: positionId }, 
            updateFields
          )
