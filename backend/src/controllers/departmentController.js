...
class DepartmentController {
  ...
  async createDepartment(req, res, next) {
    try {
      ...
      // 检查代码是否已存在
      const existingDepartment = await databaseService.findOne('departments', { 
      ...
      if (departmentData.businessLineId) {
        const businessLine = await databaseService.getBusinessLineById(departmentData.businessLineId)
        if (!businessLine) {
          return res.status(400).json({
            code: 400,
            message: '指定的业务线不存在',
      ...
      const newDepartment = await databaseService.createDepartment(departmentData)
  ...
  async updateDepartment(req, res, next) {
    try {
      ...
      // 检查部门是否存在
      const existingDepartment = await databaseService.getDepartmentById(id)
      if (!existingDepartment) {
        return res.status(404).json({
          code: 404,
          message: '部门不存在',
      ...
      if (updateData.code && updateData.code !== existingDepartment.code) {
        const codeExists = await databaseService.findOne('departments', { 
      ...
      const result = await databaseService.updateDepartment(id, updateData)

      if (result > 0) {
        const updatedDepartment = await databaseService.getDepartmentById(id)
        res.json({
          code: 200,
          data: {
            ...updatedDepartment,
  ...
  async deleteDepartment(req, res, next) {
    try {
      ...
      const result = await databaseService.deleteDepartment(id)
  ...
  async getDepartmentTree(req, res, next) {
    try {
      ...
      const departments = await databaseService.find('departments', query)
      const businessLines = await databaseService.find('businessLines', { status: 1 })
  ...
  async batchDepartments(req, res, next) {
    try {
      ...
      for (const departmentId of departmentIds) {
        try {
          const result = await databaseService.updateDepartment(departmentId, updateData)
          if (result > 0) {
            updatedCount++
          }
        } catch (error) {
