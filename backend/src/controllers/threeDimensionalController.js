...
class ThreeDimensionalController {
  ...
  async createWeightConfig(req, res) {
    try {
      // 检查是否已存在生效状态的配置
      const existingActiveConfig = await ThreeDimensionalWeightConfig.findOne({
        where: { status: 'active' }
      })
      ...
      if (existingActiveConfig) {
        return res.status(400).json({
          code: 400,
          message: '当前已存在生效状态的配置，请先停用现有配置后再创建新配置',
          data: null
        })
      }
      
      // 过滤掉不应在创建时设置的字段
      const { createdAt, created_at, updatedAt, updated_at, updatedBy, updated_by, id, _id, ...filteredBody } = req.body
  ...
  async getWeightConfigList(req, res) {
    try {
      ...
      if (rows && rows.length > 0) {
        logger.info('第一条记录的关键字段:', {
          profitContributionWeight: rows[0].profitContributionWeight,
          positionValueWeight: rows[0].positionValueWeight,
          performanceWeight: rows[0].performanceWeight,
          profit_contribution_weight: rows[0].profit_contribution_weight
        })
      }
  ...
  async updateWeightConfig(req, res) {
    try {
      const { id } = req.params
      // 过滤掉不应更新的字段
      const { createdAt, created_at, createdBy, created_by, ...filteredBody } = req.body
      ...
      // 如果正在将配置状态更新为激活，则检查是否已存在其他激活的配置
      if (updateData.status === 'active') {
        const existingActiveConfig = await ThreeDimensionalWeightConfig.findOne({
          where: { 
            status: 'active',
            id: { [Op.ne]: id } // 排除当前更新的配置
          }
        })
        ...
        if (existingActiveConfig) {
          return res.status(400).json({
            code: 400,
            message: '已存在其他生效状态的配置，请先停用现有配置后再激活此配置',
            data: null
          })
        }
      }
  
      await ThreeDimensionalWeightConfig.update(updateData, {
        where: { id: id }
      })
  ...
  async deleteWeightConfig(req, res) {
    try {
      ...
      // 使用 databaseService 查询权重配置
      const weightConfig = await databaseService.findOne('threeDimensionalWeightConfigs', { _id: id })
      if (!weightConfig) {
        return res.status(404).json({
          code: 404,
          message: '权重配置不存在'
      ...
      // 检查是否有关联的计算结果
      const hasResults = await databaseService.findOne('threeDimensionalCalculationResults', { weightConfigId: id })
      ...
      // 使用 databaseService 删除记录
      await databaseService.remove('threeDimensionalWeightConfigs', { _id: id })

      logger.info(`用户${req.user.id}删除三维权重配置: ${weightConfig.name || weightConfig._id}`)
  ...
  async copyWeightConfig(req, res) {
    try {
      ...
      const originalData = originalConfig.toJSON()
      // 过滤掉不应在新配置中保留的字段
      const { _id, createdAt, created_at, updatedAt, updated_at, updatedBy, updated_by, createdBy, created_by, ...filteredOriginalData } = originalData
