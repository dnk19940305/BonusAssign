...
class PermissionDelegationController {
  ...
  async createDelegation(req, res, next) {
    try {
      ...
      // 获取被委派者信息
      const delegatee = await databaseService.getUserById(delegateeId)
      if (!delegatee) {
        return res.status(400).json({
          code: 400,
          message: '被委派者不存在',
  ...
  async getDelegationHistory(req, res, next) {
    try {
      ...
      if (!PermissionValidator.canAccessResource(currentUser, 'permission', 'view')) {
        // 直接使用databaseService的底层查询方法
        const delegations = await databaseService.find('permissionDelegations', {
          delegatorId: currentUser.id
        });
        
        const delegations2 = await databaseService.find('permissionDelegations', {
          delegateeId: currentUser.id
        });
        
        // 合并结果
        let allDelegations = [...delegations, ...delegations2];
        
        // 去重（基于id）
        const uniqueDelegationsMap = new Map();
        allDelegations.forEach(d => {
          if (!uniqueDelegationsMap.has(d.id)) {
            uniqueDelegationsMap.set(d.id, d);
          }
        });
        allDelegations = Array.from(uniqueDelegationsMap.values());
        
        // 过滤其他查询条件
        if (status) {
          allDelegations = allDelegations.filter(d => d.status === status);
        }
        if (resourceType) {
          allDelegations = allDelegations.filter(d => d.resourceType === resourceType);
        }
        
        // 时间范围过滤
        if (startDate || endDate) {
          allDelegations = allDelegations.filter(d => {
      ...
      // 管理员用户可以查看所有委派历史，应用所有查询条件
      const delegations = await databaseService.find('permissionDelegations', query)
