...
class EmployeeController {
  ...
  async getEmployees(req, res, next) {
    try {
      ...
      // 获取所有员工
      let employees = await databaseService.getEmployees()
      if (search) {
        employees = employees.filter(emp =>
          (emp.name &&emp.name.includes(search)) ||
          (emp.employeeNo&&emp.employeeNo.includes(search)) ||
          (emp.email && emp.email.includes(search)) ||
      ...
      const employeesWithDetails = await Promise.all(
        paginatedEmployees.map(async (employee) => {
          ...
          if (department && department.businessLineId) {
            businessLine = await databaseService.getBusinessLineById(department.businessLineId)
          }
          ...
          return {
            ...
            position: position ? {
              ...
              name: position.name,
              code: position.code,
              level: position.level,  // 添加职位等级字段
              benchmarkValue: position.benchmarkValue  // 添加基准值字段
            } : null,
  ...
  async getEmployee(req, res, next) {
    try {
      ...
      const employee = await databaseService.getEmployeeById(id)
  ...
  async createEmployee(req, res, next) {
    try {
      ...
      // 确保身份证号、紧急联系人电话等字段保持字符串类型
      if (employeeData.idCard !== undefined) {
        employeeData.idCard = String(employeeData.idCard)
      }
      if (employeeData.emergencyContact !== undefined) {
        employeeData.emergencyContact = String(employeeData.emergencyContact)
      }
      if (employeeData.emergencyPhone !== undefined) {
        employeeData.emergencyPhone = String(employeeData.emergencyPhone)
      }
      ...
      // 检查员工编号是否已存在
      const existingEmployee = await databaseService.findOne('employees', {
      ...
      if (employeeData.departmentId) {
        const department = await databaseService.getDepartmentById(employeeData.departmentId)
        if (!department) {
          return res.status(400).json({
            code: 400,
            message: '指定的部门不存在',
      ...
      if (employeeData.positionId) {
        const position = await databaseService.getPositionById(employeeData.positionId)
        if (!position) {
          return res.status(400).json({
            code: 400,
            message: '指定的岗位不存在',
      ...
      if (employeeData.departmentId) {
        const department = await databaseService.getDepartmentById(employeeData.departmentId)
        if (department && department.businessLineId) {
          employeeData.businessLineId = department.businessLineId
        }
      }
  ...
  async updateEmployee(req, res, next) {
    try {
      ...
      if (updateData.idCard !== undefined) {
        updateData.idCard = String(updateData.idCard)
      }
      if (updateData.emergencyContact !== undefined) {
        updateData.emergencyContact = String(updateData.emergencyContact)
      }
      if (updateData.emergencyPhone !== undefined) {
        updateData.emergencyPhone = String(updateData.emergencyPhone)
      }
      ...
      // 检查员工是否存在
      const existingEmployee = await databaseService.getEmployeeById(id)
      if (!existingEmployee) {
        return res.status(404).json({
          code: 404,
          message: '员工不存在',
      ...
      if (updateData.employeeNo && updateData.employeeNo !== existingEmployee.employeeNo) {
        const employeeNoExists = await databaseService.find('employees', {
      ...
      if (updateData.departmentId && updateData.departmentId !== existingEmployee.departmentId) {
        const department = await databaseService.getDepartmentById(updateData.departmentId)
        if (!department) {
          return res.status(400).json({
            code: 400,
            message: '指定的部门不存在',
      ...
      if (updateData.positionId && updateData.positionId !== existingEmployee.positionId) {
        const position = await databaseService.getPositionById(updateData.positionId)
        if (!position) {
          return res.status(400).json({
            code: 400,
            message: '指定的岗位不存在',
      ...
      if (updateData.departmentId && updateData.departmentId !== existingEmployee.departmentId) {
        const department = await databaseService.getDepartmentById(updateData.departmentId)
        if (department && department.businessLineId) {
          updateData.businessLineId = department.businessLineId
        }
      }

      const result = await databaseService.updateEmployee(id, updateData)

      if (result > 0) {
        const updatedEmployee = await databaseService.getEmployeeById(id)
        res.json({
          code: 200,
          data: {
            ...updatedEmployee,
  ...
  async transfer(req, res, next) {
    try {
      ...
      // 验证部门是否存在
      if (departmentId) {
        const department = await databaseService.getDepartmentById(departmentId)
        if (!department) {
          return res.status(400).json({
            code: 400,
            message: '目标部门不存在',
      ...
      // 验证岗位是否存在
      if (positionId) {
        const position = await databaseService.getPositionById(positionId)
        if (!position) {
          return res.status(400).json({
            code: 400,
            message: '目标岗位不存在',
      ...
      if (result > 0) {
        ...
        if (updatedEmployee.departmentId) {
          department = await databaseService.getDepartmentById(updatedEmployee.departmentId)
        }

        let position = null
        if (updatedEmployee.positionId) {
          position = await databaseService.getPositionById(updatedEmployee.positionId)
        }
  ...
  async batchOperation(req, res, next) {
    try {
      ...
      for (const employeeId of employeeIds) {
        try {
          const result = await databaseService.updateEmployee(employeeId, updateData)
          if (result > 0) {
            updatedCount++
          }
        } catch (error) {
