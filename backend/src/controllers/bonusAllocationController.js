...
class BonusAllocationController {
  ...
  async createBonusAllocation(req, res, next) {
    try {
      ...
      // 检查项目是否存在
      const project = await databaseService.getProjectById(allocationData.projectId)
      if (!project) {
  ...
  async approveBonusAllocation(req, res, next) {
    try {
      ...
      // 检查项目是否存在
      const project = await databaseService.getProjectById(allocation.projectId)
      if (!project) {
        return res.status(404).json({
          code: 404,
          message: '项目不存在',
  ...
  async getBonusAllocations(req, res, next) {
    try {
      ...
      // 获取用户可访问的项目 - 使用MySQL原生查询
      // 由于数据库中可能没有直接的方法查询JSON数组中的值，我们先获取所有相关项目
      const allProjects = await databaseService.find('projects', {})
      ...
      const userProjects = allProjects.filter(project => {
        ...
        // 项目成员 (检查members数组中是否包含用户ID)
        if (project.members && Array.isArray(project.members) && project.members.includes(user.employeeId)) {
          return true
        }
        // 公开项目
      ...
      if (accessibleProjectIds.length > 0) {
        // 使用原生MySQL查询方式，通过直接的字段匹配
        // 由于databaseService不直接支持复杂的IN查询，我们通过多次查询后合并
        const allAllocationPromises = accessibleProjectIds.map(projectId => 
          databaseService.findAll('bonusAllocations', { 
            where: { projectId: projectId }
          })
        )
        
        const allAllocationResults = await Promise.all(allAllocationPromises)
        
        // 合并所有结果
        let allAllocations = []
        allAllocationResults.forEach(result => {
          const rows = result.rows || result
          allAllocations = allAllocations.concat(rows)
        })
        
        // 在应用层应用其他过滤条件
        allocations = allAllocations
        if (projectId) allocations = allocations.filter(a => a.projectId === projectId)
        if (departmentId) allocations = allocations.filter(a => a.departmentId === departmentId)
        if (employeeId) allocations = allocations.filter(a => a.employeeId === employeeId)
  ...
  async updateBonusAllocation(req, res, next) {
    try {
      ...
      // 更新奖金分配
      const updatedAllocation = await databaseService.updateBonusAllocation(allocationId, {
        ...updateData,
        updatedBy: user.id,
        updatedAt: new Date()
      })
  ...
  async deleteBonusAllocation(req, res, next) {
    try {
      ...
      // 删除奖金分配
      await databaseService.deleteBonusAllocation(allocationId)
